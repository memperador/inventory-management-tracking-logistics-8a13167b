  ...
  return (
    <Card className={`bg-gradient-to-b ${getTierGradient()} ${className}`}>
      <CardContent className="flex flex-col items-center text-center p-6 space-y-4">
        <Zap className={`w-10 h-10 ${requiredTier === 'premium' ? 'text-purple-500' : requiredTier === 'enterprise' ? 'text-slate-600' : 'text-blue-500'}`} />
        ...
        <Button 
          className={`mt-4 ${requiredTier === 'premium' ? 'bg-purple-600 hover:bg-purple-700' : requiredTier === 'enterprise' ? 'bg-slate-800 hover:bg-slate-900' : ''}`}
